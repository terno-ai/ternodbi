"""
Validation services for DBI Layer.
"""
...
def validate_datasource_input(type_, conn_str, connection_json=None):
    """
    Validate datasource connection parameters.
    
    Args:
        type_: Database type (postgres, mysql, etc.)
        conn_str: Connection string
        connection_json: Optional JSON credentials (for BigQuery)
    ...
    # Prevent double protocol (e.g., bigquery://bigquery://)
    if conn_str.count('://') > 1:
    ...
    if drivername.startswith('bigquery'):
        ...
        try:
            ...
            # Validate BigQuery project/dataset format
            ...
            # Try to validate with BigQuery client if available
            try:
                ...
                # google-cloud-bigquery not installed, fall through to connector test
    ...
    # Try to connect using ConnectorFactory
    try:
